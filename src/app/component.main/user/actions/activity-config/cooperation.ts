...
@Component({
    selector : 'activity-cooperation-config',
...
export class ActivityCooperationConfigComponent implements OnInit{
    ngOnInit(): void {
    }
    //预览的初始位置
    previewPos = 0;
    ...
    getConfig(){
        let config = {
            ...
            isLockPhoneInput: this.pageConfig.isLockPhoneInput == '1',
            wxShare: {
                isCustomize: this.pageConfig.wxShare.isCustomize == '1',
                title: this.pageConfig.wxShare.title,
                content: this.pageConfig.wxShare.content,
                icon: this.pageConfig.wxShare.icon,
                url: this.pageConfig.wxShare.url
            }
